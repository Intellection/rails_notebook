...
railsNB.renderFlamechart  = function( _data , _element ){
  require( ["/kernelspecs/rails_notebook/d3.js", "jquery"] , function ( d3, $ ) {
  ...
  var generate_flamechart = function () {
      ...
      $(element).append('<svg version="1.1" class="chart" id='+element.context.id+'></svg>');
...
railsNB.renderRoutes      = function( tree , element ){
  require(["jquery", "/kernelspecs/rails_notebook/jquery.tipsy.js", "/kernelspecs/rails_notebook/dagre-d3.js", "/kernelspecs/rails_notebook/d3.js"], function ( $, tipsy , dagreD3 , d3 ) {
...
railsNB.renderSchema      = function( tables, element ){
  require(["jquery", "/kernelspecs/rails_notebook/dagre-d3.js", "/kernelspecs/rails_notebook/d3.js"], function ( $ , dagreD3 , d3 ) {
      ...
      for (var i=tablesVarLangth-1; i >= 0; i--)
      {
          ...
          buildHtmlOutput = buildHtmlOutput + "<table class=schemaTable border='0' cellspacing='0' cellpadding='0' style='color:#66c1ff;font-style:italic; border:0px solid' >";
...
railsNB.renderTableData      = function( tableData, element ){
  require(["jquery", "/kernelspecs/rails_notebook/dagre-d3.js", "/kernelspecs/rails_notebook/d3.js"], function ( $ , dagreD3 , d3 ) {
      ...
      //Create rectangles for each of the tables
      var numberOfColumns = tableDataVar.length;
      var numberOfRows = tableDataVar[1].length; // First row is just the column name
      var buildHtmlOutput = "<div style = 'text-align: center;'><span style='font-size:32px;color:#007acc;font-style:italic;'><u><font style = 'font-family:Palatino'>" + tableDataVar[0] + "</font></u></span>";
      buildHtmlOutput = buildHtmlOutput + "<table class=queryTable border='0' cellspacing='0' cellpadding='0' style='color:#66c1ff;font-style:italic; border:0px solid' >";
      for (var i=0; i < numberOfRows; i++)
      {
        buildHtmlOutput = buildHtmlOutput + "<tr>";
        for (var j=1; j < numberOfColumns; j++)
        {
            buildHtmlOutput = buildHtmlOutput + "<td>" + tableDataVar[j][i] + "</td>";
        }
        buildHtmlOutput = buildHtmlOutput + "</tr>";
      }
      // for (var i=0; i < tableDataVarLangth; i++)
      // {
      //     tableDataVar[i].columns.forEach(function(column){
      //         var column_name = column[0];
      //         var column_type = column[1];
      //         buildHtmlOutput = buildHtmlOutput + "<tr><td>" + column_name + "</td><td>" + column_type + "</td></tr>";
      //     });
      ...
      // } // End of loop i, but not end of scope for i
      buildHtmlOutput += "</table></div>";
      g.setNode("tableQuery", { shape: "rect", labelType: "html", label: buildHtmlOutput});
